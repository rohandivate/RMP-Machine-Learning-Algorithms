...
public class LinearClassifier {
	...
	public double training() {
		...
		while(iter < MaxIter) {
			...
			for(int i = 0; i < m; i++) {
				...
				for(int nj = 0; nj < trainData.elementAt(i).size(); nj++) {
					int j = trainData.elementAt(i).elementAt(nj);
					dw[j] += pen[L]*(h[i]-trainLabel.elementAt(i));
				}
				db += pen[L]*(h[i]-trainLabel.elementAt(i));
			}
		...
		try {
	        BufferedWriter out = new BufferedWriter(new FileWriter("/Users/rdivate2016/Desktop/LinearParameters-example.txt"));
	...
	public void predict() {
		...
		try {
			String line;
			FileReader file = new FileReader("/Users/rdivate2016/Desktop/LinearParameters-2.txt");
			BufferedReader b = new BufferedReader(file);
			int count = 0;
			while((line = b.readLine()) != null) {
	...
	public static void main(String[] args) {
		String filename = "/Users/rdivate2016/Desktop/mcf-7-gvec-2.txt";
		LinearClassifier test = new LinearClassifier();
		test.loadData(filename);
		test.shuffle();
		//for(int k = 0; k < 10; k++) {
			// get k-th fold training and test data
			test.makeTrainingData(0);
